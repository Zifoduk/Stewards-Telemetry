...
def Race_Director(self, parent_frame):
    ...
    self.frame_6 = customtkinter.CTkFrame(parent_frame,width=500)
    self.frame_6.grid(row=1,column=1,padx=20,pady=0,sticky='news')


    self.label2 = customtkinter.CTkLabel(self.frame_6,textvariable=globalVars.activated_StringVar,width=800)
    self.label2.grid(row=2, column=0,padx=10,pady=10)

    self.activate = customtkinter.CTkButton(self.frame_6,text='toggle connection',height=30,width=90,command=lambda: Handle_Toggle_Connection(self))
